...
/*
  Simple test refining a quadralateral, validating recalculating hmin
  and (further) splitting subsegments in chew_uniform.
*/
...
int main(int argc, char* argv[]) {
    ...
    umr::mesh::Point ps0(0.25, 0.25);
    umr::mesh::Point ps1(0.75, 0.75);
    umr::mesh::Point pc(0.5, 0.5);
    double radius = 0.3;
    double hmin = 1;

    auto line = std::make_shared<umr::parametric::Line>(ps0, ps1);
    auto arc = std::make_shared<umr::parametric::Arc>(pc, radius,
            std::numbers::pi / 4, std::numbers::pi * 5 / 4);
    arc->set_num_subsegments({2});
    ...
    // full mesh is not unique; only test points
    is_valid = validate_points(m1, 2);
    if (!is_valid) {
        std::cout << "Aborting\n";
        return EXIT::FAIL;
    }
